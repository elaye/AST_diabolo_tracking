...
bool colorTracking::update(ofPixels pixels, ofPoint * pDiabolo){
    ...
    //    colorImg.setFromPixels(vidGrabber.getPixels(), camWidth, camHeight);    //remember that colorImg? put the camera image into it
        colorImg.setFromPixels(pixels.getPixels(), camWidth, camHeight);    //remember that colorImg? put the camera image into it
    ...
    be represented by certain hue ranges. hues from 4 to 21 are redish while 109 to 115 are green*/
        ...
        for (int i = 0; i < nPixels; i++){                                           //let's go through every pixel in hue pixels
            if ((huePixels[i] >= one.hue - 12 && huePixels[i] <= one.hue + 12) &&    //if the hue is of a certain range
                (satPixels[i] >= one.sat - 24 && satPixels[i] <= one.sat + 24)){    //if the saturation is of a certain range
    ...
    //------------------------------
        ...
        if(finderRed.blobs.size() > 0) {
        	if(jitter == true){
                xj = 0.5*xj + 0.5*finderRed.blobs[0].centroid.x;
                yj = 0.5*yj + 0.5*finderRed.blobs[0].centroid.y;            
            }
            else{
        	...
        	pDiabolo->x = xj / camWidth;
        	pDiabolo->y = yj / camHeight;
            return true;
        }
    }
    return false;
...
//ofEnableAlphaBlending();
    ...
    if(colorSelected){
        ...
        //trackedTextureRed.draw(camWidth, 480);
        ...
        ofPushStyle();
            ofNoFill();
            ofSetColor(ofColor::fromHsb(one.hue, one.sat, 100));
            ofBeginShape();
...
void colorTracking::selectColor(ofColor color, int x, int y){
    ...
    //one.hue = color.getHue();
    //one.sat = color.getSaturation();
